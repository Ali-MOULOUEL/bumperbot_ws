...
class OdometryMotionModel(Node):
    ...
    def __init__(self):
        ...
        self.is_firt_odom = True
        self.last_odom_x = 0.0
        self.last_odom_y = 0.0
        self.last_odom_theta = 0.0

        self.declare_parameter('alpha1', 0.1) # First parameter of noise effet for odometry motion model 
        self.declare_parameter('alpha2', 0.1)
        self.declare_parameter('alpha3', 0.1)
        self.declare_parameter('alpha4', 0.1)
        self.declare_parameter('nr_samples', 300) # Number of samples to be generated particles for odometry motion model postion estimation
        ...
        if self.nr_samples >= 0:
            ...
            self.get_logger().error("Invalid number of samples requested: %d", self.nr_samples)
            return
    ...
    def odomCallback(self, odom):
        ...
        delta_transl = sqrt(pow(odom_y_increment, 2) + pow(odom_x_increment, 2))
        ...
        for sample in self.samples.poses:
            # each particule will be affected by diffirent noise
