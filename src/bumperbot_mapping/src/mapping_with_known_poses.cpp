...
{
    declare_parameter("width", 50.0);
    declare_parameter("height", 50.0);
    ...
    propability_map_ = std::vector<double>(map_.info.width * map_.info.height, prob2logodds(PRIOR_PRO));

    map_pub_ = create_publisher<nav_msgs::msg::OccupancyGrid>("map", 1);
    scan_sub_ = create_subscription<sensor_msgs::msg::LaserScan>("scan", 1, std::bind(&MappingWithKnownPoses::scanCallback, this, _1));
...
void MappingWithKnownPoses::scanCallback(const sensor_msgs::msg::LaserScan & scan)
{
    ...
    try{
        ...
        RCLCPP_ERROR(get_logger(), "Unable to transform between /odom and /base_foot_print");
        return;
    }

    Pose robot_p = coordinateToPose(t.transform.translation.x, t.transform.translation.y, map_.info);
    if(!poseOnMap(robot_p, map_.info))
    {
        RCLCPP_ERROR(get_logger(), "Robot pose is outside the map");
        return;
    }
    ...
    for(size_t i = 0; i < scan.ranges.size(); ++i){
        ...
        Pose beam_p = coordinateToPose(px, py, map_.info);
        ...
        for(const auto & pose : poses)
        {
            if(poseOnMap(pose.first, map_.info)){
                ...
                propability_map_.at(cell) += prob2logodds(pose.second) - prob2logodds(PRIOR_PRO);
            }
        }
    }
}
...
{ 
    ...
    std::transform(propability_map_.begin(), propability_map_.end(), map_.data.begin(), [](double value){
        return logodds2prob(value) * 100.0; 
    });
