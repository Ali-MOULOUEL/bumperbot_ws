...
{
public:
  SafetyStop() : Node("safety_stop_node"), state_(FREE), prev_state_(FREE)
  {
    ...
    while(!decrease_speed_client_->wait_for_action_server(std::chrono::seconds(1)) && rclcpp::ok()){
      RCLCPP_INFO(get_logger(), "Waiting for action server joy_turbo_decrease to start.");
      std::this_thread::sleep_for(std::chrono::seconds(2));
    }
    while(!increase_speed_client_->wait_for_action_server(std::chrono::seconds(1)) && rclcpp::ok()){
      RCLCPP_INFO(get_logger(), "Waiting for action server joy_turbo_increase to start.");
      std::this_thread::sleep_for(std::chrono::seconds(2));
    }
